...
fail:
    Py_XDECREF(int_str);
    cRETURN;
//  TODO uses string conversion for now, which avoids an issue on Windows
//    iENTER;
//
...
iERR ionc_write_value(hWRITER writer, PyObject* obj, PyObject* tuple_as_sexp) {
    ...
    if (PyUnicode_Check(obj)) {
        ...
        IONCHECK(ion_writer_write_typed_null(writer, (ION_TYPE)ion_type));
          ...
          // TODO using string-decimal conversion for now, which avoids unnecessary error on Windows.
//        if (ion_type == tid_none_INT) {
...
//            //TODO what should be the right size
//            char dec_str[60]; // DECQUAD_String is only 43, which fails some tests.
