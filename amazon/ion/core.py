...
# TODO At some point we can add SCALAR_START/SCALAR_END for streaming large
# values.
class IonEventType(Enum):
    """Enumeration of Ion parser or serializer events.

    These types do not correspond directly to the Ion type system, but they are
    related. In particular, ``null.*`` will surface as a ``SCALAR`` even though
    they are containers.

    Attributes:
        INCOMPLETE: Indicates that parsing cannot be completed due to lack of
            input.
        STREAM_END: Indicates that the logical stream has terminated.
        VERSION_MARKER: Indicates that the **Ion Version Marker** has been
            encountered.
        SCALAR: Indicates an *atomic* value has been encountered.
        CONTAINER_START: Indicates that the start of a container has been
            reached.
    ...
    def begins_value(self):
        ...
        return self is IonEventType.SCALAR or \
            self is IonEventType.CONTAINER_START

    @property
    def ends_container(self):
        """Indicates if the event type terminates a container or stream."""
        return self is IonEventType.STREAM_END or \
            self is IonEventType.CONTAINER_END
...
class IonEvent(
    ...
    Args:
        ...
        field_name (Optional[Union[amazon.ion.symbols.SymbolToken, unicode]]):
    ...
    def __eq__(self, other):
        ...
        if isinstance(self.value, float):
            ...
            if self.value != other.value and not (
                    isnan(self.value) and isnan(other.value)):
                return False
        else:
            ...
            if isinstance(self.value, Timestamp) or isinstance(
                    other.value, Timestamp):
                # Special case for timestamps to capture equivalence over
                # precision.
                self_precision = getattr(
                    self.value, TIMESTAMP_PRECISION_FIELD, None)
                other_precision = getattr(
                    other.value, TIMESTAMP_PRECISION_FIELD, None)

                if self_precision != other_precision and not (
                        (
                            self_precision is None and other_precision is
                            TimestampPrecision.SECOND) or
                        (
                            self_precision is TimestampPrecision.SECOND and
                            other_precision is None)):
                    # The absence of precision indicates a naive datetime,
                    # which always has SECOND precision.
                    return False
    ...
    def derive_field_name(self, field_name):
        ...
        Args:
            field_name (Union[amazon.ion.symbols.SymbolToken, unicode]): The
            field name to set.
        Returns:
    ...
    def derive_annotations(self, annotations):
        """
        Derives a new event from this one setting the ``annotations``
        attribute.

        Args:
            annotations: (Sequence[Union[amazon.ion.symbols.SymbolToken,
                unicode]]):
...
class MemoizingThunk(object):
    """
    A :class:`callable` that invokes a ``delegate`` and caches and returns the
    result.
    """
...
class DataEvent(record('type', 'data')):
    ...
    Args:
        ...
        data (bytes):  The serialized data returned.  If no data is to be
            serialized, this should be the empty byte string.
    """
...
class Transition(record('event', 'delegate')):
    ...
    Args:
        ...
        delegate (Coroutine): The co-routine delegate which can be the same
            routine from whence this transition came.
    """
...
class Timestamp(datetime):
    ...
    Notes:
        The ``precision`` field is passed as a keyword argument of the same
        name.
    """
    ...
    def __new__(cls, *args, **kwargs):
        ...
        if TIMESTAMP_FRACTION_PRECISION_FIELD in kwargs:
            ...
            if fractional_precision is not None and \
                    1 > fractional_precision > MICROSECOND_PRECISION:
                raise ValueError(
                    'Cannot construct a Timestamp with fractional precision '
                    'of %d digits, which is out of the supported range of '
                    '[1, %d].' % (
        ...
        setattr(
            instance, TIMESTAMP_FRACTION_PRECISION_FIELD, fractional_precision)
    ...
    def __repr__(self):
        return 'Timestamp(%04d-%02d-%02dT%02d:%02d:%02d.%06d, %r, %r, %s=%s)' \
            % (
                ...
                self.second, self.microsecond, self.tzinfo, self.precision,
                TIMESTAMP_FRACTION_PRECISION_FIELD, self.fractional_precision)

    @staticmethod
    def adjust_from_utc_fields(*args, **kwargs):
        """
        Constructs a timestamp from UTC fields adjusted to the local offset if
        given.
        """
        ...
        # XXX This returns a datetime, not a Timestamp (which has our precision
        # if defined)
...
def timestamp(year, month=1, day=1,
    ...
    Specifically, converts ``off_hours`` and ``off_minutes`` parameters to a
    suitable :class:`OffsetTZInfo` instance.
    """
    delta = None
    if off_hours is not None:
        if off_hours < -23 or off_hours > 23:
            raise ValueError(
                'Hour offset %d is out of required range -23..23.' %
                (off_hours,))
        delta = timedelta(hours=off_hours)
    if off_minutes is not None:
        if off_minutes < -59 or off_minutes > 59:
            raise ValueError(
                'Minute offset %d is out of required range -59..59.' %
                (off_minutes,))
    ...
    if microsecond is not None:
        ...
        if fractional_precision is not None:
            raise ValueError(
                'Cannot have fractional precision without a fractional '
                'component.')
