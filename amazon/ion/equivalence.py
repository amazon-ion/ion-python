...
"""
Provides utilities for determining whether two objects are equivalent under the
Ion data model.
"""
...
from amazon.ion.core import IonType, Timestamp, TimestampPrecision, \
    MICROSECOND_PRECISION, OffsetTZInfo
from amazon.ion.simple_types import _IonNature, IonPyList, IonPyDict, \
    IonPyTimestamp, IonPyNull, IonPySymbol, IonPyText, IonPyDecimal, IonPyFloat
...
def ion_equals(a, b, timestamps_instants_only=False):
    ...
    There are three important cases:
        * When neither operand specifies its `ion_type` or `annotations`, this
          method will only return True when the values of both operands are
          equivalent under the Ion data model.
        * When only one of the operands specifies its `ion_type` and
          `annotations`, this method will only return True when that operand
          has no annotations and has a value equivalent to the other operand
          under the Ion data model.
        * When both operands specify `ion_type` and `annotations`, this method
          will only return True when the ion_type and annotations of both are
          the same and their values are equivalent under the Ion data model.
    ...
    Args:
        ...
        timestamps_instants_only (Optional[bool]): False if timestamp objects
            (datetime and its subclasses) should be compared according to the
            Ion data model (where the instant, precision, and offset must be
            equal); True if these objects should be considered equivalent if
            they simply represent the same instant.
    """
...
def _ion_equals_timestamps_instants(a, b):
    return _ion_equals(
        a, b, _timestamp_instants_eq, _ion_equals_timestamps_instants)
    ...
    for a, b in ((a, b), (b, a)):
        if isinstance(a, _IonNature):
            if isinstance(b, _IonNature):
                # Both operands have _IonNature. Their IonTypes and annotations
                # must be equivalent.
                eq = a.ion_type is b.ion_type and _annotations_eq(a, b)
            else:
                # Only one operand has _IonNature. It cannot be equivalent to
                # the other operand if it has annotations.
            ...
            if eq:
                if isinstance(a, IonPyList):
                    ...
                    return isinstance(b, IonPyNull) or (
                        b is None and a.ion_type is IonType.NULL)
                elif isinstance(a, IonPySymbol) or (
                        isinstance(a, IonPyText) and
                        a.ion_type is IonType.SYMBOL):
    ...
    for a, b in ((a, b), (b, a)):
        if isinstance(a, Timestamp):
            if isinstance(b, Timestamp):
                # Both operands declare their precisions. They are only
                # equivalent if their precisions are the same.
                if a.precision is b.precision and \
                        a.fractional_precision is b.fractional_precision:
                    break
                return False
            elif a.precision is not TimestampPrecision.SECOND or \
                    a.fractional_precision != MICROSECOND_PRECISION:
                # Only one of the operands declares its precision. It is only
                # equivalent to the other (a naive datetime) if it has full
...
def _timestamp_instants_eq(a, b):
    ...
    if not isinstance(b, datetime):
        return False
    # datetime's __eq__ can't compare a None offset and a non-None offset. For
    # these equivalence semantics, a None
    ...
    # datetime's __eq__ implementation compares instants; offsets and
    # precision need not be equal.
    return a == b
...
def _symbols_eq(a, b):
    ...
    if a_text == b_text:
        if a_text is None:
            # Both have unknown text. If they come from a local context, they
            # are equivalent.
            ...
            if a_location is not None:
                # Both were imported from shared symbol tables. In this case,
                # they are only equivalent if they were imported from the same
                # position in the same shared symbol table.
                if (a_location.name != b_location.name) or (
                        a_location.position != b_location.position):
                    return False
            a_sid = getattr(a, 'sid', None)
            b_sid = getattr(b, 'sid', None)
            if a_sid is None or b_sid is None:
                raise ValueError(
                    'Attempted to compare malformed symbols %s, %s.' % (a, b))
            if (a_sid == 0) ^ (b_sid == 0):
