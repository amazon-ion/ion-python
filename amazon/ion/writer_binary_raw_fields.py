...
def _write_varint(buf, value):
    ...
    Args:
        buf (Sequence): The buffer into which the VarInt will be written, in the form of integer octets.
...
def _write_int(buf, value):
    ...
    Args:
        buf (Sequence): The buffer into which the Int will be written, in the form of integer octets.
...
def _write_varuint(buf, value):
    ...
    Args:
        buf (Sequence): The buffer into which the VarUInt will be written, in the form of integer octets.
...
def _write_uint(buf, value):
    ...
    Args:
        buf (Sequence): The buffer into which the UInt will be written, in the form of integer octets.
...
def _write_base(buf, value, bits_per_octet, end_bit=0, sign_bit=0, is_signed=False):
    ...
    buf (Sequence): The buffer into which the UInt will be written, in the form of integer octets.
    value (int): The value to write as a UInt.
    bits_per_octet (int): The number of value bits (i.e. exclusive of the end bit, but inclusive of the sign bit,
                          if applicable) per octet.
    ...
    if remainder != 0 or is_signed:  # If signed, the first octet has one fewer bit available, requiring another octet.
        num_octets += 1
    else:
        remainder = bits_per_octet  # This ensures that unsigned values that fit exactly are not shifted too far.
    for i in range(num_octets):
        ...
        if i == num_octets - 1:
            octet |= end_bit
        # 'remainder' is used for alignment such that only the first octet may contain insignificant zeros.
...
class _FieldCache:
    ...
    For unsigned fields, values between 0 and 63 will have their representations cached. For signed fields, values
    between -32 and 31 will have their representations cached. It is likely that a large proportion of subfields fit in
    these ranges. For example:
    - All values with lengths less than 64 bytes will retrieve their length subfield from the cache.
    - All integer values with magnitudes less than 64 will retrieve their 'magnitude' subfields from the cache.
    - Decimal values with exponents and/or coefficients within the signed cache range will retrieve those fields from
      the cache.
    - All timestamps will retrieve their month, day, hour, minute, and second fields from the cache (otherwise it would
      be an invalid datetime). The fractional fields may be cached if they meet the decimal value requirement from the
      previous bullet.
    - The first 64 symbols will have their symbol ID fields retrieved from the cache. This applies to symbol values,
      field names, and annotations.
    - If a value has less than 64 bytes of annotations (hopefully this is true for all values), then its annotation
      wrapper's 'annotation length' field will be cached.
    """
    @classmethod
    ...
    def _fill_cache(self):
        # Each cached_value is an index, but since the caches are self-resizing sequences, they are not directly used
        # that way here. Each _write* method below appends to the given cache. Since each cache starts empty, the field
        # value ends up at index 'cached_value' in the cache. For unsigned fields, the cached field represents
        # cached_value directly. In other words, the UInt cache contains the UInt field representation of '32' at index
        # '32'. For signed fields, the cached field represents (cached_value - 32) to give a selection of positive and
        # negative values. In other words, the Int cache contains the Int field representation of '0' at index '32'.
        #
        # The size of 64 was chosen because it is the maximum magnitude that fits in a 1-byte VarInt field (the smallest
        # max magnitude of any 1-byte field representation). Therefore, each value of 'cached_value' is guaranteed to
        # fit in one byte for any of the four field types (this is verified by the assertion). Note that different sizes
        # could be used for each of the caches, but for simplicity, 64 is used for all.
