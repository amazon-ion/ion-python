...
def _invalid_handler(type_octet, _):
...
def _var_uint_field_handler(handler, context: ParserContext):
    ...
    return handler(length, ParserContext(buffer, depth))


def _static_scalar_handler(ion_type, value, context: ParserContext):
    return IonEvent(IonEventType.SCALAR, ion_type, value, depth=context.depth), context.buffer


def _length_scalar_handler(scalar_factory, ion_type, length, context: ParserContext):
...
def _nop_pad_handler(_, length, context: ParserContext):
    if not length:
...
def _ivm_handler(context: ParserContext):
    (buffer, depth) = context
    if depth:
...
def _annotation_handler(_, length, context: ParserContext):
    (buffer, depth) = context
    ...
    event, buffer = _tlv_parser(ParserContext(buffer, depth))
...
def _container_start_handler(ion_type, length, context: ParserContext):
...
class _ContainerStartEvent(IonEvent):
    limit: int

def _ordered_struct_start_handler(length, context: ParserContext):
    if length < 2:
...
def _ivm_parser(context: ParserContext):
    """
    For IVMs only
    """
    ...
    return _ivm_handler(ParserContext(buffer, depth))


def _tlv_parser(context: ParserContext):
    """

    """
    (buffer, depth) = context
    (tid, buffer) = buffer.read_byte()
    return _HANDLER_DISPATCH_TABLE[tid](ParserContext(buffer, depth))


def _struct_item_parser(context: ParserContext):
    """
    ...
    if not event:
        # todo: i think we need some validation about nops and no field names or something...
        return event, buffer
    else:
        return event.derive_field_name(SymbolToken(None, field_sid)), buffer


Parser = Callable[[ParserContext], Tuple[IonEvent, int]]


class ContextFrame(NamedTuple):
    parser: Parser
...
def stream_handler():
    """
    Handler for the top-level stream.
    """
    buffer = SliceableBuffer()
    context_stack = deque([ContextFrame(_tlv_parser, None, 0, -2)])
    # position in stream
    cursor = 0
    ion_event = None
    ...
    # The first part handles the users' request, checking invariants and
    # mutating state as needed.
    # The second part parses if possible/needed then mutates state based
    # on the results of parsing (or not).
    while True:
        read_event = yield ion_event
        assert read_event is not None

        ### handle user's read event ###
        if expect_data:
        ...
        ### produce ion event and parse if needed ###

        # loop is to consume but suppress no-ops
        while not ion_event:
            ...
            if cursor == limit:
                ...
                # todo: i don't know where this is tested
                raise IonException("Passed limit of current container!")
            # parsing is fun, let's do that!
            else:
                try:
                    (ion_event, new_buff) = parser(ParserContext(buffer, depth))
        ...
        if event_type.is_stream_signal:
            ...
            if event_type is IonEventType.CONTAINER_START:
                if ion_event.ion_type is IonType.STRUCT:
                    ...
                    parser = _tlv_parser
                # we're abusing "value" for the length of the container
                frame = ContextFrame(parser, ion_event.ion_type, depth + 1, cursor + ion_event.value)
...
_HANDLER_DISPATCH_TABLE: List[Callable] = [None] * 256
