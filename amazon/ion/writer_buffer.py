...
class BufferTree:
    ...
    Containers are treated as subtrees. Calling start_container creates an empty node which represents
    the start of a new subtree, while calling end_container steps out of that subtree, allowing for
    additional nodes to be added as siblings of that subtree's start node. It is important that there
    is exactly one call to start_container per call to end_container. Nodes representing the start of
    a container subtree are kept in a stack; upon calling end_container, the subtree represented by the
    node that was created by the most recent call to start_container will be stepped out. Other than
    that, no container semantics are asserted by this class.

    Scalar values are treated as child nodes of the current container subtree (which may be at the
    top-level if the current container subtree's root node is the root node of the BufferTree). Calling
    add_scalar_value will add a new node as a child of the current container subtree. A container
    subtree's children are ordered.

    If None values are passed in to end_container or add_scalar_value, nodes will be added to the tree,
    in the same way as described above, but no value will be yielded for that node upon drain.
    """
    def __init__(self):
        self.__root = None
