...
from amazon.ion.core import Transition, ION_STREAM_INCOMPLETE_EVENT, \
    ION_STREAM_END_EVENT, IonType, IonEvent, IonEventType, IonThunkEvent, \
    TimestampPrecision, timestamp, ION_VERSION_MARKER_EVENT, \
    MICROSECOND_PRECISION
from amazon.ion.exceptions import IonException
from amazon.ion.reader import BufferQueue, reader_trampoline, ReadEventType, \
    safe_unichr, CodePointArray, _NARROW_BUILD
from amazon.ion.symbols import SymbolToken, TEXT_ION_1_0
from amazon.ion.util import record, coroutine, Enum, _next_code_point, \
    CodePoint
...
def _illegal_character(c, ctx, message=''):
    """
    Raises an IonException upon encountering the given illegal character in the
    given context.

    Args:
        c (int|None): Ordinal of the illegal character.
        ctx (_HandlerContext):  Context in which the illegal character was
            encountered.
        message (Optional[str]): Additional information, as necessary.

    """
    container_type = ctx.container.ion_type is None and 'top-level' or \
        ctx.container.ion_type.name
    ...
    raise IonException(
        '%s at position %d in %s value contained in %s. %s Pending value: %s' %
        (
            header, ctx.queue.position, value_type, container_type, message,
            ctx.value))


def _defaultdict(dct, fallback=_illegal_character):
    """
    Wraps the given dictionary such that the given fallback function will be
    called when a nonexistent key is accessed.
    """
...
def _merge_mappings(*args):
    ...
    If a given argument is a tuple, it must have two elements, the first of
    which is a sequence of keys and the second of which is a single value,
    which will be mapped to from each of the keys in the sequence.
    """
    dct = {}
    for arg in args:
...
# NOTE: the following are stored as sequences of integer code points. This
# simplifies dealing with inconsistencies between how bytes objects are handled
# in python 2 and 3, and simplifies logic around comparing multi-byte
# characters.
...
# The following suffixes are used for comparison when a token is found that
# starts with the first letter in the keyword. For example, when a new token
# starts with 't', the next three characters must match those in _TRUE_SUFFIX,
# followed by an acceptable termination character, in order for the token to
# match the 'true' keyword.
...
class _NullSequence:
    """
    Contains the terminal character sequence for the typed null suffix of the
    given IonType, starting with the first character after the one which
    disambiguated the type.

    For example, SYMBOL's _NullSequence contains the characters
    'mbol' because 'null.s' is ambiguous until 'y' is found, at which point it
    must end in 'mbol'.
...
# The following implements a prefix tree used to determine whether a typed null
# keyword has been found (see _typed_null_handler). The leaves of the tree
# (enumerated above) are the terminal character sequences for the 13 possible
# suffixes to 'null.'. Any other suffix to 'null.' is an error. _NULL_STARTS is
# entered when 'null.' is found.
...
_NULL_STARTS = {
    ...
    # null.string, null.symbol, null.struct, null.sexp
    _ord(b's'): _NULL_S_NEXT,
...
class _ContainerContext(
    ...
    """
    A description of an Ion container, including the container's IonType and
    its textual delimiter and end character, if applicable.

    This is tracked as part of the current token's context, and is useful when
    certain lexing decisions depend on which container the token is a member
    of. For example, ending a numeric token with ']' is not legal unless that
    token is contained in a list.

    Args:
        end (tuple): Tuple containing the container's end character, if any.
        delimiter (tuple): Tuple containing the container's delimiter
        character, if any.
...
def _is_escaped(c):
    """
    Queries whether a character ordinal or code point was part of an escape
    sequence.
    """
    try:
        return c.is_escaped
...
def _as_symbol(value, is_symbol_value=True):
    ...
    part of a :class:`IonEvent`.

    If the input has an `as_symbol` method (e.g. :class:`CodePointArray`), it
    will be converted using that method.
    Otherwise, it must already be a `SymbolToken`. In this case, there is
    nothing to do unless the input token is not a symbol value and it is an
    :class:`_IVMToken`. This requires the `_IVMToken` to be converted to a
    regular `SymbolToken`.
    """
    try:
    ...
    if not is_symbol_value:
        try:
            # This converts _IVMTokens to regular SymbolTokens when the
            # _IVMToken cannot represent an IVM (i.e. it is a field name or
            # annotation).
...
class _HandlerContext():
    ...
    Args:
        container (_ContainerContext): The description of the container in
        which this context is contained.
        ...
        value (Optional[bytearray|CodePointArray]): The (in-progress) value of
            this context's token.
        ion_type (Optional[IonType]): The IonType of the current token.
        pending_symbol (Optional[bytearray|CodePointArray]): A pending symbol,
            which may end up being an annotation, field name, or symbol value.
        quoted_text (Optional[bool]): True if this context represents quoted
            text; otherwise, False.
        line_comment (Optional[bool]): True if this context represents a line
            comment; otherwise, False.
        code_point (Optional[int|CodePoint]): The token's current unicode code
            point, if applicable.
        is_self_delimiting (Optional[bool]): True if this context's token is
            self-delimiting (a short string, container, or comment).
        is_composite (Optional[bool]): True if this context's token is a value
            immediately followed by another token discovered during lookahead.
    """

    def __init__(
            self, container, queue, field_name, annotations, depth, whence,
            value, ion_type, pending_symbol, quoted_text=False,
            line_comment=False, code_point=None, is_self_delimiting=False,
            is_composite=False):
    ...
    def event_transition(self, event_cls, event_type, ion_type, value):
        ...
        if ion_type is IonType.SYMBOL:
            if not annotations and depth == 0 and isinstance(value, _IVMToken):
                ...
                if event is None:
                    _illegal_character(
                        None, self, 'Illegal IVM: %s.' % (value.text,))
    ...
    def read_data_event(self, whence, complete=False, can_flush=False):
        ...
        Args:
            whence (Coroutine): The co-routine to return to after the data is
                satisfied.
            complete (Optional[bool]): True if STREAM_END should be emitted if
                no bytes are read or available; False if INCOMPLETE should be
                emitted in that case.
            can_flush (Optional[bool]): True if NEXT may be requested after
                INCOMPLETE is emitted as a result of this data request.
        """
        return Transition(
            None, _read_data_handler(whence, self, complete, can_flush))
    ...
    def set_unicode(self, quoted_text=False):
        """
        Converts the context's ``value`` to a sequence of unicode code points
        for holding text tokens, indicating whether the text is quoted.
        """
    ...
    def set_quoted_text(self, quoted_text):
        """
        Sets the context's ``quoted_text`` flag. Useful when entering and
        exiting quoted text tokens.
        """
        self.quoted_text = quoted_text
        self.line_comment = False
        return self
    ...
    def set_self_delimiting(self, is_self_delimiting):
        """
        Sets the context's ``is_self_delimiting`` flag. Useful when the end of
        a self-delimiting token (short string, container, or comment) is
        reached.

        This is distinct from the ``quoted_text`` flag because some quoted text
        (quoted symbols and long strings) are not self-delimiting--they require
        lookahead to determine if they are complete.
        """
        self.is_self_delimiting = is_self_delimiting
        return self
    ...
    def set_code_point(self, code_point):
        """
        Sets the context's current ``code_point`` to the given ``int`` or
        :class:`CodePoint`.
        """
        self.code_point = code_point
        return self
    ...
    def set_empty_symbol(self):
        """
        Resets the context, retaining the fields that make it a child of its
        container (``container``, ``queue``, ``depth``, ``whence``), and sets
        an empty ``pending_symbol``.

        This is useful when an empty quoted symbol immediately follows a long
        string.
        """
    ...
    def set_line_comment(self, is_line_comment=True):
        """
        Sets the context's ``line_comment`` flag. Useful when entering or
        exiting a line comment.
        """
    ...
    def set_annotation(self):
        """
        Appends the context's ``pending_symbol`` to its ``annotations``
        sequence.
        """
        ...
        self.annotations = annotations if not self.annotations else \
            self.annotations + annotations
        self.ion_type = None
    ...
    def set_field_name(self):
        ...
        self.field_name = _as_symbol(
            self.pending_symbol, is_symbol_value=False)
        self.pending_symbol = None  # reset pending symbol
        self.quoted_text = False
        self.line_comment = False
        self.is_self_delimiting = False
    ...
    def set_pending_symbol(self, pending_symbol=None):
        """
        Sets the context's ``pending_symbol`` with the given unicode sequence
        and resets the context's ``value``.
...
class _CompositeTransition(Transition):
    """
    Composes an event transition followed by an immediate transition to the
    handler for the next token.

    This is useful when some lookahead is required to determine if a token has
    ended, e.g. in the case of long strings.

    Args:
        event_transition (Transition): A transition with a non-None IonEvent.
        current_context (_HandlerContext): The context for the value contained
            in ``event_transition``.
        next_handler (Coroutine): The handler that will lex the next token.
            Only None if ``next_context`` contains a complete token (as is the
            case with an empty quoted symbol following a long string).
        next_context (Optional[_HandlerContext]): The context for the next
            token. If None, a new child context will be derived from ``ctx``.
        initialize_handler (Optional[bool]): True if the ``next_handler``
            coroutine needs to be initialized; otherwise, False.
    """
    def __new__(cls, event_transition, *args, **kwargs):
        return Transition.__new__(
            cls, event_transition.event, event_transition.delegate)

    def __init__(
            self, event_transition, current_context, next_handler,
            next_context=None, initialize_handler=True):
        assert event_transition.event is not None
        if next_context is None:
            next_context = current_context.derive_child_context(
                current_context.whence)
...
_parse_decimal = partial(
    _parse_number, partial(_base_10, Decimal, decode=True))


@coroutine
def _number_negative_start_handler(c, ctx):
    """
    Handles numeric values that start with a negative sign. Branches to
    delegate co-routines according to _NEGATIVE_TABLE.
    """
    assert c == _MINUS
...
def _number_zero_start_handler(c, ctx):
    """
    Handles numeric values that start with zero or negative zero. Branches to
    delegate co-routines according to _ZERO_START_TABLE.
    """
    assert c == _ZERO
    assert len(ctx.value) == 0 or (
        len(ctx.value) == 1 and ctx.value[0] == _MINUS)
    ctx.set_ion_type(IonType.INT)
    ctx.value.append(c)
    c, _ = yield
    if _ends_value(c):
        trans = ctx.event_transition(
            IonThunkEvent, IonEventType.SCALAR, ctx.ion_type,
            _parse_decimal_int(ctx.value))
        if c == _SLASH:
            trans = ctx.immediate_transition(
                _number_slash_end_handler(c, ctx, trans))
        yield trans
...
def _number_or_timestamp_handler(c, ctx):
    """
    Handles numeric values that start with digits 1-9. May terminate a value,
    in which case that value is an int. If it does not terminate a value, it
    branches to delegate co-routines according to _NUMBER_OR_TIMESTAMP_TABLE.
    """
    assert c in _DIGITS

    # If this is the last digit read, this value is an Int.
    ctx.set_ion_type(IonType.INT)
    ...
    while True:
        if _ends_value(c):
            ...
            if c not in _DIGITS:
                trans = ctx.immediate_transition(
                    _NUMBER_OR_TIMESTAMP_TABLE[c](c, ctx))
            else:
                val.append(c)
        c, _ = yield trans
...
def _number_slash_end_handler(c, ctx, event):
    """
    Handles numeric values that end in a forward slash. This is only legal if
    the slash begins a comment; thus, this co-routine either results in an
    error being raised or an event being yielded.
    """
    assert c == _SLASH
    c, self = yield
    ...
    # If the previous line returns without error, it's a valid comment and the
    # number may be emitted.
    yield _CompositeTransition(
        event, ctx, comment, next_ctx, initialize_handler=False)


def _numeric_handler_factory(
        charset, transition, assertion, illegal_before_underscore, parse_func,
        illegal_at_end=(None,), ion_type=None, append_first_if_not=None,
        first_char=None):
    """
    Generates a handler co-routine which tokenizes a numeric component (a token
    or sub-token).

    Args:
        charset (sequence): Set of ordinals of legal characters for this
        numeric component.
        transition (callable): Called upon termination of this component (i.e.
            when a character not in ``charset`` is found). Accepts the previous
            character ordinal, the current character ordinal, the current
            context, and the previous transition. Returns a Transition if the
            component ends legally; otherwise, raises an error.
        assertion (callable): Accepts the first character's ordinal and the
            current context. Returns True if this is a legal start to the
            component.
        illegal_before_underscore (sequence): Set of ordinals of illegal
            characters to precede an underscore for this component.
        parse_func (callable): Called upon ending the numeric value. Accepts
            the current token value and returns a thunk that lazily parses the
            token.
        illegal_at_end (Optional[sequence]): Set of ordinals of characters that
            may not legally end the value.
        ion_type (Optional[IonType]): The type of the value if it were to end
            on this component.
        append_first_if_not (Optional[int]): The ordinal of a character that
            should not be appended to the token if it occurs first in this
            component (e.g. an underscore in many cases).
        first_char (Optional[int]): The ordinal of the character that should be
            appended instead of the character that occurs first in this
            component. This is useful for preparing the token for parsing in
            the case where a particular character is peculiar to the Ion format
            (e.g. 'd' to denote the exponent of a decimal value should be
            replaced with 'e' for compatibility with python's Decimal type).
    """
    @coroutine
        ...
        while True:
            if _ends_value(c):
                if prev == _UNDERSCORE or prev in illegal_at_end:
                    _illegal_character(
                        c, ctx, '%s at end of number.' % (_chr(prev),))
                trans = ctx.event_transition(
                    IonThunkEvent, IonEventType.SCALAR, ctx.ion_type,
                    parse_func(ctx.value))
                if c == _SLASH:
                    trans = ctx.immediate_transition(
                        _number_slash_end_handler(c, ctx, trans))
            else:
                if c == _UNDERSCORE:
                    if prev == _UNDERSCORE or \
                            prev in illegal_before_underscore:
                        _illegal_character(
                            c, ctx, 'Underscore after %s.' % (_chr(prev),))
                else:
                    if c not in charset:
...
def _exponent_handler_factory(
        ion_type, exp_chars, parse_func, first_char=None):
    ...
    Args:
        ...
        exp_chars (sequence): The set of ordinals of the legal exponent
            characters for this component.
        parse_func (callable): Called upon ending the numeric value. Accepts
            the current token value and returns a thunk that lazily parses the
            token.
        first_char (Optional[int]): The ordinal of the character that should be
    ...
    return _numeric_handler_factory(
        _DIGITS, transition, lambda c, ctx: c in exp_chars, illegal,
        parse_func, illegal_at_end=illegal, ion_type=ion_type,
        first_char=first_char)


_decimal_handler = _exponent_handler_factory(
    IonType.DECIMAL, _DECIMAL_EXPS, _parse_decimal, first_char=_ord(b'e'))
_float_handler = _exponent_handler_factory(
    IonType.FLOAT, _FLOAT_EXPS, _parse_float)


def _coefficient_handler_factory(
        trans_table, parse_func, assertion=lambda c, ctx: True, ion_type=None,
        append_first_if_not=None):
    """Generates a handler co-routine which tokenizes a numeric coefficient.

    Args:
        trans_table (dict): lookup table for the handler for the next
            component of this numeric token, given the ordinal of the first
            character in that component.
        parse_func (callable): Called upon ending the numeric value. Accepts
            the current token value and returns a thunk that lazily parses the
            token.
        assertion (callable): Accepts the first character's ordinal and the
    ...
    return _numeric_handler_factory(
        _DIGITS, transition, assertion, (_DOT,), parse_func, ion_type=ion_type,
        append_first_if_not=append_first_if_not)
...
fractional_number_handler = _coefficient_handler_factory(
    _FRACTIONAL_NUMBER_TABLE, _parse_decimal,
    assertion=lambda c, ctx: c == _DOT, ion_type=IonType.DECIMAL)
...
_whole_number_handler = _coefficient_handler_factory(
    _WHOLE_NUMBER_TABLE, _parse_decimal_int, append_first_if_not=_UNDERSCORE)


def _radix_int_handler_factory(radix_indicators, charset, parse_func):
    """
    Generates a handler co-routine which tokenizes a integer of a particular
    radix.

    Args:
        radix_indicators (sequence): The set of ordinals of characters that
        indicate the radix of this int.
    ...
    def assertion(c, ctx):
        return c in radix_indicators and (
            (len(ctx.value) == 1 and ctx.value[0] == _ZERO) or (
                len(ctx.value) == 2 and ctx.value[0] == _MINUS and
                ctx.value[1] == _ZERO)) and ctx.ion_type == IonType.INT
    return _numeric_handler_factory(
        charset, lambda prev, c, ctx, trans: _illegal_character(c, ctx),
        assertion, radix_indicators, parse_func,
        illegal_at_end=radix_indicators)


_binary_int_handler = _radix_int_handler_factory(
    _BINARY_RADIX, _BINARY_DIGITS, _parse_binary_int)
_hex_int_handler = _radix_int_handler_factory(
    _HEX_RADIX, _HEX_DIGITS, _parse_hex_int)


@coroutine
def _timestamp_zero_start_handler(c, ctx):
    """
    Handles numeric values that start with a zero followed by another digit.
    This is either a timestamp or an error.
    """
    val = ctx.value
...
class _TimestampTokens:
    """
    Holds the individual numeric tokens (as strings) that compose a
    `Timestamp`.
    """
...
def _parse_timestamp(tokens):
    """
    Parses each token in the given `_TimestampTokens` and marshals the numeric
    components into a `Timestamp`.
    """
    def parse():
            ...
            if fraction is not None:
                ...
                if fraction_digits > MICROSECOND_PRECISION:
                    for digit in fraction[MICROSECOND_PRECISION:]:
                        if digit != _ZERO:
                            raise ValueError(
                                'Only six significant digits supported in '
                                'timestamp fractional. Found %s.' %
                                (fraction,))
                    fraction_digits = MICROSECOND_PRECISION
                    fraction = fraction[0:MICROSECOND_PRECISION]
                else:
                    fraction.extend(
                        _ZEROS[MICROSECOND_PRECISION - fraction_digits])
...
def _timestamp_handler(c, ctx):
    """
    Handles timestamp values. Entered after the year component has been
    completed; tokenizes the remaining components.
    """
    ...
    if len(ctx.value) != 4:
        _illegal_character(
            c, ctx,
            'Timestamp year is %d digits; expected 4.' % (len(ctx.value),))
    prev = c
    c, self = yield
    trans = ctx.immediate_transition(self)
    ...
    while True:
        ...
        if c not in nxt and not is_eof:
            _illegal_character(
                c, ctx,
                'Expected %r in state %r.' % ([_chr(x) for x in nxt], state))
        if c in _VALUE_TERMINATORS or is_eof:
            if not can_terminate:
                _illegal_character(
                    c, ctx, 'Unexpected termination of timestamp.')
            trans = ctx.event_transition(
                IonThunkEvent, IonEventType.SCALAR, ctx.ion_type,
                _parse_timestamp(tokens))
            if c == _SLASH:
                trans = ctx.immediate_transition(
                    _number_slash_end_handler(c, ctx, trans))
        else:
            ...
            if c == _Z:
                ...
                if prev == _PLUS or (
                        state > _TimestampState.MONTH and prev == _HYPHEN):
                    ...
                    if state == _TimestampState.MONTH:
                        ...
                        raise ValueError(
                            'Unknown timestamp state %r.' % (state,))
                else:
                    # Reaching this branch would be indicative of a programming
                    # error within this state machine.
                    raise ValueError(
                        'Digit following %s in timestamp state %r.' %
                        (_chr(prev), state))
                val.append(c)
        prev = c
        c, _ = yield trans
...
def _comment_handler(c, ctx, whence):
    """
    Handles comments. Upon completion of the comment, immediately transitions
    back to `whence`.
    """
    assert c == _SLASH
    c, self = yield
    if c == _SLASH:
        ...
        if ctx.line_comment:
            # This happens when a block comment immediately follows a line
            # comment.
            ctx.set_line_comment(False)
        block_comment = True
    else:
        _illegal_character(
            c, ctx, 'Illegal character sequence "/%s".' % (_chr(c),))
    done = False
    prev = None
    trans = ctx.immediate_transition(self)
    while not done:
...
def _sexp_slash_handler(c, ctx, whence=None, pending_event=None):
    """
    Handles the special case of a forward-slash within an s-expression. This is
    either an operator or a comment.
    """
    assert c == _SLASH
    if whence is None:
        whence = ctx.whence
    ...
    if c == _ASTERISK or c == _SLASH:
        ...
        if pending_event is not None:
            # Since this is the start of a new value and not a comment, the
            # pending event must be emitted.
            assert pending_event.event is not None
            yield _CompositeTransition(
                pending_event, ctx, partial(_operator_symbol_handler, _SLASH))
...
def _validate_quoted_text(allowed_whitespace, c, ctx, max_char):
    if c not in allowed_whitespace and not _is_escaped(c) and \
        ...
        _illegal_character(
            c, ctx, 'Character out of range [%d, %d] for this type.'
...
def _long_string_handler(c, ctx, is_field_name=False):
    """
    Handles triple-quoted strings. Remains active until a value other than a
    long string is encountered.
    """
    ...
    while True:
            ...
            if in_data:
                ...
                if quotes > 0:
                    ...
                    if is_field_name or is_clob:
                        # There are at least two values here, which is illegal
                        # for field names or within clobs.
                        _illegal_character(
                            c, ctx, 'Malformed triple-quoted text: %s' %
                            (val,))
                    else:
                        # This string value is followed by a quoted symbol.
                        if ctx.container.is_delimited:
                            _illegal_character(
                                c, ctx, 'Delimiter %s not found after value.'
                                % (_chr(ctx.container.delimiter[0]),))
                        trans = ctx.event_transition(
                            IonEvent, IonEventType.SCALAR, ctx.ion_type,
                            ctx.value.as_text())
                        if quotes == 1:
                            if BufferQueue.is_eof(c):
                                _illegal_character(c, ctx, "Unexpected EOF.")
                            # c was read as a single byte. Re-read it as a
                            # code point.
                            ctx.queue.unread(c)
                            ...
                            trans = _CompositeTransition(
                                ...
                                ctx,
                                partial(
                                    _quoted_symbol_handler, c,
                                    is_field_name=False),)
                        else:  # quotes == 2
                            trans = _CompositeTransition(
                                trans, ctx, None, ctx.set_empty_symbol())
                elif c not in _WHITESPACE:
                    if is_clob:
                        trans = ctx.immediate_transition(
                            _clob_end_handler(c, ctx))
                    elif c == _SLASH:
                        if ctx.container.ion_type is IonType.SEXP:
                            pending = ctx.event_transition(
                                IonEvent, IonEventType.SCALAR, ctx.ion_type,
                                ctx.value.as_text())
                            trans = ctx.immediate_transition(
                                _sexp_slash_handler(c, ctx, self, pending))
                        else:
                            trans = ctx.immediate_transition(
                                _comment_handler(c, ctx, self))
                    elif is_field_name:
                        if c != _COLON:
                            _illegal_character(
                                c, ctx,
                                'Illegal character after field name %s.' %
                                (val,))
        ...
        # If comments separated long string components, this would have been
        # set.
        ctx.set_self_delimiting(False)
        trans = here
...
def _typed_null_handler(c, ctx):
    ...
    while True:
        if done:
            if _ends_value(c) or (
                    ctx.container.ion_type is IonType.SEXP and
                    c in _OPERATORS):
                trans = ctx.event_transition(
                    IonEvent, IonEventType.SCALAR, nxt.ion_type, None)
            else:
...
def _symbol_or_keyword_handler(c, ctx, is_field_name=False):
    ...
    This may be an operator (if in an s-expression), an identifier symbol, or
    a keyword.
    """
    ...
    while True:
        def check_keyword(
                name, keyword_sequence, ion_type, value,
                match_transition=lambda: None):
            maybe_keyword = True
            transition = None
            if match_index < len(keyword_sequence):
                ...
                if transition is not None:
                    ...
                    if is_field_name:
                        _illegal_character(
                            c, ctx, '%s keyword as field name not allowed.' %
                            (name,))
                    transition = ctx.event_transition(
                        IonEvent, IonEventType.SCALAR, ion_type, value)
                elif c == _COLON:
                    message = ''
                    if is_field_name:
                        message = '%s keyword as field name not allowed.' % (
                            name,)
        ...
        if maybe_null:
            def check_null_dot():
                ...
                if found:
                    if is_field_name:
                        _illegal_character(
                            c, ctx, "Illegal character in field name.")
                    transition = ctx.immediate_transition(
                        _typed_null_handler(c, ctx))
                return transition
            maybe_null, keyword_trans = check_keyword(
                'null', _NULL_SUFFIX.sequence, IonType.NULL, None,
                check_null_dot)
        if maybe_nan:
            maybe_nan, keyword_trans = check_keyword(
                'nan', _NAN_SUFFIX, IonType.FLOAT, _NAN)
        elif maybe_true:
            maybe_true, keyword_trans = check_keyword(
                'true', _TRUE_SUFFIX, IonType.BOOL, True)
        elif maybe_false:
            maybe_false, keyword_trans = check_keyword(
                'false', _FALSE_SUFFIX, IonType.BOOL, False)
            ...
            if c in _SYMBOL_TOKEN_TERMINATORS:
                ...
                trans = ctx.immediate_transition(
                    _unquoted_symbol_handler(
                        c, ctx, is_field_name=is_field_name))
        c, _ = yield trans
...
def _inf_or_operator_handler_factory(c_start, is_delegate=True):
    ...
    Args:
        c_start (int): The ordinal of the character that starts this token
        (either `+` or `-`).
        is_delegate (bool): True if a different handler began processing this
            token; otherwise, False. This will only be true for `-inf`, because
            it is not the only value that can start with `-`; `+inf` is the
            only value (outside of a s-expression) that can start with `+`.
    """
    @coroutine
    def inf_or_operator_handler(c, ctx):
                ...
                if match_index < len(_INF_SUFFIX):
                    ...
                    if _ends_value(c) or (
                        ...
                        yield ctx.event_transition(
                            IonEvent, IonEventType.SCALAR, IonType.FLOAT,
                            c_start == _MINUS and _NEG_INF or _POS_INF)
                    else:
                        maybe_inf = False
            if maybe_inf:
                match_index += 1
        ...
        if ctx.container is not _C_SEXP:
            _illegal_character(
                c, next_ctx is None and ctx or next_ctx,
                'Illegal character following %s.' % (_chr(c_start),))
        if match_index == 0:
            if c in _OPERATORS:
                yield ctx.immediate_transition(
                    _operator_symbol_handler(c, ctx))
            yield ctx.event_transition(
                IonEvent, IonEventType.SCALAR, IonType.SYMBOL,
                ctx.value.as_symbol())
        yield _CompositeTransition(
            ctx.event_transition(
                IonEvent, IonEventType.SCALAR, IonType.SYMBOL,
                ctx.value.as_symbol()), ctx, partial(
                    _unquoted_symbol_handler, c), next_ctx)
    return inf_or_operator_handler


_negative_inf_or_sexp_hyphen_handler = _inf_or_operator_handler_factory(_MINUS)
_positive_inf_or_sexp_plus_handler = _inf_or_operator_handler_factory(
    _PLUS, is_delegate=False)
...
def _operator_symbol_handler(c, ctx):
    ...
    while c in _OPERATORS:
        ...
        c, _ = yield trans
    yield ctx.event_transition(
        IonEvent, IonEventType.SCALAR, IonType.SYMBOL, val.as_symbol())
...
def _symbol_token_end(c, ctx, is_field_name, value=None):
    ...
    if is_field_name or c in _SYMBOL_TOKEN_TERMINATORS or ctx.quoted_text:
        # This might be an annotation or a field name. Mark it as
        # self-delimiting because a symbol token termination
        # character has been found.
        ctx.set_self_delimiting(ctx.quoted_text).set_pending_symbol(
            value).set_quoted_text(False)
        trans = ctx.immediate_transition(ctx.whence)
    else:
        trans = ctx.event_transition(
...
def _unquoted_symbol_handler(c, ctx, is_field_name=False):
    """
    Handles identifier symbol tokens. If in an s-expression, these may be
    followed without whitespace by operators.
    """
    in_sexp = ctx.container.ion_type is IonType.SEXP
    ctx.set_unicode()
    ...
    while True:
        if c not in _WHITESPACE:
            if prev in _WHITESPACE or _ends_value(c) or c == _COLON or (
                    in_sexp and c in _OPERATORS):
                break
            if c not in _IDENTIFIER_CHARACTERS:
                _illegal_character(c, ctx.set_ion_type(IonType.SYMBOL))
            val.append(c)
...
class _IVMToken(SymbolToken):
    """
    Subclass of :class:`SymbolToken`, which indicates that this token's text
    matches the IVM pattern.
    """
    def ivm_event(self):
        """
        If this token's text is a supported IVM, returns the :class:`IonEvent`
        representing that IVM. Otherwise, returns `None`.
        """
        try:
    ...
    def regular_token(self):
        ...
        This will be used in _as_symbol when this token is used as an
        annotation or field name, in which cases it can no longer be an IVM.
        """
...
def _symbol_identifier_or_unquoted_symbol_handler(c, ctx, is_field_name=False):
    """
    Handles symbol tokens that begin with a dollar sign. These may end up being
    system symbols ($ion_*), symbol identifiers ('$' DIGITS+), or regular
    unquoted symbols.
    """
    ...
    while True:
        if c not in _WHITESPACE:
            ...
            if maybe_ivm:
                ...
                yield ctx.immediate_transition(
                    _unquoted_symbol_handler(c, ctx, is_field_name))
            val.append(c)
...
_validate_short_quoted_text = partial(
    _validate_quoted_text, _WHITESPACE_NOT_NL)


def _quoted_text_handler_factory(
        delimiter, assertion, before, after, append_first=True,
        on_close=lambda ctx: None):
    """
    Generates handlers for quoted text tokens (either short strings or quoted
    symbols).

    Args:
        delimiter (int): Ordinal of the quoted text's delimiter.
        assertion (callable): Accepts the first character's ordinal, returning
            True if that character is a legal beginning to the token.
        before (callable): Called upon initialization. Accepts the first
            character's ordinal, the current context, True if the token is a
            field name, and True if the token is a clob; returns the token's
            current value and True if ``on_close`` should be called upon
            termination of the token.
        after (callable): Called after termination of the token. Accepts the
            final character's ordinal, the current context, and True if the
            token is a field name; returns a Transition.
        append_first (Optional[bool]): True if the first character the
            coroutine receives is part of the text data, and should therefore
            be appended to the value; otherwise, False (in which case, the
            first character must be the delimiter).
        on_close (Optional[callable]): Called upon termination of the token
            (before ``after``), if ``before`` indicated that ``on_close``
            should be called. Accepts the current context and returns a
            Transition. This is useful for yielding a different kind of
            Transition based on initialization parameters given to ``before``
            (e.g.  string vs. clob).
    """
    @coroutine
...
def _short_string_handler_factory():
    ...
    def on_close(ctx):
        ...
        return ctx.event_transition(
            IonEvent, IonEventType.SCALAR, ctx.ion_type, ctx.value.as_text())
    ...
    return _quoted_text_handler_factory(
        _DOUBLE_QUOTE, lambda c: c == _DOUBLE_QUOTE, before, after,
        append_first=False, on_close=on_close)
...
def _single_quote_handler_factory(on_single_quote, on_other):
    """
    Generates handlers used for classifying tokens that begin with one or more
    single quotes.

    Args:
        on_single_quote (callable): Called when another single quote is found.
            Accepts the current character's ordinal, the current context, and
            True if the token is a field name; returns a Transition.
        on_other (callable): Called when any character other than a single
            quote is found.  Accepts the current character's ordinal, the
            current context, and True if the token is a field name; returns a
            Transition.
    """
    @coroutine
...
_two_single_quotes_handler = _single_quote_handler_factory(
    lambda c, ctx, is_field_name: ctx.set_unicode(
        quoted_text=True).immediate_transition(
            _long_string_handler(c, ctx, is_field_name)),
    lambda c, ctx, is_field_name: ctx.set_ion_type(
        IonType.SYMBOL).set_pending_symbol().immediate_transition(ctx.whence)
    # Empty symbol.
    )
_long_string_or_symbol_handler = _single_quote_handler_factory(
    lambda c, ctx, is_field_name:
        ctx.set_ion_type(IonType.SYMBOL).immediate_transition(
            _two_single_quotes_handler(c, ctx, is_field_name)),
    lambda c, ctx, is_field_name: ctx.immediate_transition(
        _quoted_symbol_handler(c, ctx, is_field_name)))
...
def _b64decode_py2(value):
    # Some versions of python 2 don't support bytearray as input to
    # base64.b64decode.
...
def _lob_end_handler_factory(
        ion_type, action, validate=lambda c, ctx, action_res: None):
    """Generates handlers for the end of blob or clob values.

    Args:
        ion_type (IonType): The type of this lob (either blob or clob).
        action (callable): Called for each non-whitespace, non-closing brace
            character encountered before the end of the lob. Accepts the
            current character's ordinal, the current context, the previous
            character's ordinal, the result of the previous call to ``action``
            (if any), and True if this is the first call to ``action``.
            Returns any state that will be needed by subsequent calls to
            ``action``. For blobs, this should validate the character is valid
            base64; for clobs, this should ensure there are no illegal
            characters (e.g. comments) between the end of the data and the end
            of the clob.
        validate (Optional[callable]): Called once the second closing brace has
            been found. Accepts the current character's ordinal, the current
            context, and the result of the last call to ``action``; raises an
            error if this is not a valid lob value.
    """
    ...
    def lob_end_handler(c, ctx):
        ...
        while True:
            if c in _WHITESPACE:
                if prev == _CLOSE_BRACE:
                    _illegal_character(
                        c, ctx.set_ion_type(ion_type), 'Expected }.')
            elif c == _CLOSE_BRACE:
                if prev == _CLOSE_BRACE:
        ...
        yield ctx.event_transition(
            IonThunkEvent, IonEventType.SCALAR, ion_type,
            _parse_lob(ion_type, val))
    return lob_end_handler


def _blob_end_handler_factory():
    """
    Generates the handler for the end of a blob value. This includes the
    ...
    def validate(c, ctx, res):
        ...
        if num_pads > 3 or (num_digits + num_pads) % 4 != 0:
            _illegal_character(
                c, ctx,
                'Incorrect number of pad characters (%d) for a blob of %d '
                'base-64 digits.' % (num_pads, num_digits))
...
def _clob_end_handler_factory():
    """
    Generates the handler for the end of a clob value. This includes anything
    from the data's closing quote through the second closing brace.
    """
    def action(c, ctx, prev, res, is_first):
...
_single_quoted_field_name_handler = partial(
    _long_string_or_symbol_handler, is_field_name=True)
_double_quoted_field_name_handler = partial(
    _short_string_handler, is_field_name=True)
_unquoted_field_name_handler = partial(
    _symbol_or_keyword_handler, is_field_name=True)
_symbol_identifier_or_unquoted_field_name_handler = partial(
    _symbol_identifier_or_unquoted_symbol_handler, is_field_name=True)


def _container_start_handler_factory(
        ion_type, before_yield=lambda c, ctx: None):
    """
    Generates handlers for tokens that begin with container start characters.

    Args:
        ion_type (IonType): The type of this container.
        before_yield (Optional[callable]): Called at initialization. Accepts
        the first character's ordinal and the current context; performs any
        necessary initialization actions.
    """
...
# Struct requires unread_byte because we had to read one char past the { to
# make sure it wasn't a lob.
_struct_handler = _container_start_handler_factory(
    IonType.STRUCT, lambda c, ctx: ctx.queue.unread(c))
...
def _read_data_handler(whence, ctx, complete, can_flush):
    ...
    Args:
        ...
        complete (True|False): True if STREAM_END should be emitted if no bytes
            are read or available; False if INCOMPLETE should be emitted in
            that case.
        can_flush (True|False): True if NEXT may be requested after INCOMPLETE
            is emitted as a result of this data request.
    """
    trans = None
    queue = ctx.queue
    ...
    while True:
        ...
        if data_event is not None:
            if data_event.data is not None:
                ...
                if not can_flush:
                    _illegal_character(
                        queue.read_byte(), ctx, "Unexpected EOF.")
                yield Transition(None, whence)
        trans = Transition(
...
_FIELD_NAME_START_TABLE = _defaultdict(
    ...
    fallback=partial(
        _illegal_character, message='Illegal character in field name.'))
...
def _container_handler(c, ctx):
    """
    Coroutine for container values. Delegates to other coroutines to tokenize
    all child values.
    """
    _, self = (yield None)
    queue = ctx.queue
    ...
    def symbol_value_event():
        return child_context.event_transition(
            IonEvent, IonEventType.SCALAR, IonType.SYMBOL,
            _as_symbol(child_context.pending_symbol))

    def pending_symbol_value():
        if has_pending_symbol():
            assert not child_context.value
            if ctx.ion_type is IonType.STRUCT and \
                    child_context.field_name is None:
                _illegal_character(
                    c, ctx, 'Encountered STRUCT value %s without field name.'
    ...
    def is_value_decorated():
        return child_context is not None and (
                child_context.annotations or
                child_context.field_name is not None)

    def _can_flush():
        return child_context is not None and child_context.depth == 0 and (
               (
                   child_context.ion_type is not None and (
                       child_context.ion_type.is_numeric or (
                           child_context.ion_type.is_text and
                           not ctx.quoted_text and not is_field_name))) or (
                   child_context.line_comment and
                   not is_value_decorated()))

    while True:
        # Loop over all values in this container.
        if c in ctx.container.end or c in ctx.container.delimiter or \
                BufferQueue.is_eof(c):
            ...
            if c in ctx.container.end:
                if not delimiter_required and is_value_decorated():
                    _illegal_character(
                        c, child_context,
                        'Dangling field name (%s) and/or annotation(s) (%r) '
                        'at end of container.' % (
                            child_context.field_name,
                            child_context.annotations))
                # Yield the close event and go to enclosing container. This
                # coroutine instance will never resume.
                yield Transition(
                    IonEvent(
                        IonEventType.CONTAINER_END, ctx.ion_type,
                        depth=ctx.depth-1), ctx.whence)
                raise ValueError('Resumed a finished container handler.')
            elif c in ctx.container.delimiter:
                if not delimiter_required:
                    _illegal_character(
                        c, ctx.derive_child_context(None),
        ...
        if c is not None and c not in _WHITESPACE:
            ...
            if c == _SLASH:
                if child_context is None:
                    # This is the start of a new child value (or, if this is a
                    # comment, a new value will start after the comment ends).
                    child_context = ctx.derive_child_context(self)
                if ctx.ion_type is IonType.SEXP:
                    handler = _sexp_slash_handler(
                        c, child_context, pending_event=pending_symbol_value())
                else:
                    handler = _comment_handler(c, child_context, self)
            elif delimiter_required:
                # This is not the delimiter, or whitespace, or the start of a
                # comment. Throw.
                _illegal_character(
                    c, ctx.derive_child_context(None),
                    'Delimiter %s not found after value.'
                    % (_chr(ctx.container.delimiter[0]),))
            elif has_pending_symbol():
                # A character besides whitespace, comments, and delimiters has
                # been found, and there is a pending symbol. That pending
                # symbol is either an annotation, a field name, or a symbol
                # value.
                if c == _COLON:
                    if is_field_name:
                        is_field_name = False
                        ...
                        if c == _COLON:
                            ...
                            # forces another character to be read safely
                            c = None
                        else:
                            # Colon that doesn't indicate a field name or
                            # annotation.
                    ...
                    if is_field_name:
                        _illegal_character(
                            ...
                            'Illegal character after field name %s.'
                            % child_context.pending_symbol)
                    # It's a symbol value delimited by something other than a
                    # comma (i.e. whitespace or comment)
                ...
                if is_field_name:
                    ...
                    handler = _VALUE_START_TABLE[c](c, child_context)

                    can_flush = _IMMEDIATE_FLUSH_TABLE[c]

            # _OPEN_BRACE might start a lob; that is handled elsewhere.
            container_start = c == _OPEN_BRACKET or c == _OPEN_PAREN

            quoted_start = c == _DOUBLE_QUOTE or c == _SINGLE_QUOTE
            while True:
                # Loop over all characters in the current token. A token is
                # either a non-symbol value or a pending symbol, which may end
                # up being a field name, annotation, or symbol value.
                if container_start:
                    c = None
                    ...
                    if child_context.quoted_text or quoted_start:
                        ...
                        if len(queue) == 0:
                            yield ctx.read_data_event(
                                self, can_flush=can_flush)
                        c = queue.read_byte()
                trans = handler.send((c, handler))
                if trans.event is not None:
                    is_self_delimiting = False
                    if child_context.is_composite:
                        # This is a composite transition, i.e. it is an event
                        # transition followed by an immediate transition to the
                        # handler coroutine for the next token.
                        next_transition = trans.next_transition
                        child_context = trans.next_context
                        assert next_transition is None or \
                            next_transition.event is None
                    else:
                        next_transition = None
                        is_self_delimiting = child_context.is_self_delimiting
                        child_context = None
                    # This child value is finished. c is now the first
                    # character in the next value or sequence.
                    # Hence, a new character should not be read; it should be
                    # provided to the handler for the next child context.
                    yield trans

                    # None in the case of IVM event.
                    event_ion_type = trans.event.ion_type

                    is_container = event_ion_type is not None and \
                        event_ion_type.is_container and \
                    ...
                    if is_container:
                        ...
                        yield Transition(
                            None,
                            _container_handler(
                                c, ctx.derive_container_context(
                                    trans.event.ion_type, self)))
                    complete = ctx.depth == 0
                    can_flush = False
                    if is_container or is_self_delimiting:
                        # The end of the value has been reached, and c needs to
                        # be updated
                        assert not ctx.quoted_text
                        if len(queue) == 0:
                            yield ctx.read_data_event(
                    ...
                    # The next token will determine the type.
                    child_context.set_ion_type(None)
                    ...
                    if is_field_name:
                        ...
                        if not child_context.is_self_delimiting or \
                                child_context.line_comment:
                            break
                    elif child_context.is_self_delimiting:
                        # This is the end of a comment. If this is at the top
                        # level and is un-annotated, it may end the stream.
                        complete = ctx.depth == 0 and not is_value_decorated()
                    # This happens at the end of a comment within this
                    # container, or when a symbol token has been found. In both
                    # cases, an event should not be emitted. Read the next
                    # character and continue.
                    if len(queue) == 0:
                        yield ctx.read_data_event(self, complete, can_flush)
                    c = queue.read_byte()
                    break
                # This is an immediate transition to a handler (may be the same
...
def _skip_trampoline(handler):
    """
    Intercepts events from container handlers, emitting them only if they
        ...
        if data_event is not None and data_event.type is ReadEventType.SKIP:
            while True:
                ...
                if event is not None:
                    if event.event_type is IonEventType.CONTAINER_END and \
                            event.depth <= depth:
                        break
                if event is None or \
                        event.event_type is IonEventType.INCOMPLETE:
                    data_event, _ = yield Transition(event, self)
        else:
            trans, delegate, event = pass_through()
            if event is not None and (
                    event.event_type is IonEventType.CONTAINER_START or
    ...
    while True:
        ...
        if code_point == _BACKSLASH:
            ...
            while True:
                ...
                if six.indexbytes(escape_sequence, -1) == _BACKSLASH:
                    if code_point == _ord(b'u') and unicode_escapes_allowed:
                        ...
                        if code_point == _SLASH or \
                                code_point == _QUESTION_MARK:
                            # Drop the \. Python does not recognize these as
                            # escapes.
                            escape_sequence = b''
                        ...
                        # This is a backslash followed by an invalid escape
                        # character. This is illegal.
                        _illegal_character(
                            code_point, ctx, 'Invalid escape sequence \\%s.' %
                            (_chr(code_point),))
                    escape_sequence += six.int2byte(code_point)
                else:
            ...
            if not escaped_newline:
                decoded_escape_sequence = escape_sequence.decode(
                    'unicode-escape')
                cp_iter = _next_code_point_iter(
                    decoded_escape_sequence, iter(decoded_escape_sequence),
                    to_int=ord)
                code_point = next(cp_iter)
                if code_point is None:
                    # This is a high surrogate. Restart the loop to gather the
                    # low surrogate.
            ...
            _illegal_character(
                code_point, ctx, 'Unpaired high surrogate escape sequence %s.'
                % (escape_sequence,))
...
def reader(queue=None, is_unicode=False):
    ...
    Args:
        ...
        is_unicode (Optional[bool]): True if all input data to this reader will
            be of unicode text type; False if all input data to this reader
            will be of binary type.
    ...
    Yields:
        IonEvent: parse events, will have an event type of ``INCOMPLETE`` if
            ...
            ``NEXT`` or ``SKIP`` to iterate over values; ``DATA`` or ``NEXT``
            if the last event type was ``INCOMPLETE``; or ``DATA`` if the last
            event type was ``STREAM_END``.

            When the reader receives ``NEXT`` after yielding ``INCOMPLETE``,
            this signals to the reader that no further data is coming, and that
            any pending data should be flushed as either parse events or
            errors. This is **only** valid at the top-level, and will **only**
            result in a parse event if the last character encountered...
                * was a digit or a decimal point in a non-timestamp,
                  non-keyword numeric value; OR
                * ended a valid partial timestamp; OR
                * ended a keyword value (special floats, booleans, ``null``,
                  and typed nulls); OR
                * was part of an unquoted symbol token, or whitespace or the
                  end of a comment following an unquoted symbol token (as long
                  as no colons were encountered after the token); OR
                * was the closing quote of a quoted symbol token, or whitespace
                  or the end of a comment following a quoted symbol token (as
                  long as no colons were encountered after the token); OR
                * was the final closing quote of a long string, or whitespace
                  or the end of a comment following a long string.
            If the reader successfully yields a parse event as a result of
            this, ``NEXT`` is the only input that may immediately follow. At
            that point, there are only two possible responses from the reader:
                * If the last character read was the closing quote of an empty
                  symbol following a long string, the reader will emit a parse
                  event representing a symbol value with empty text.
                  The next reader input/output event pair must be (``NEXT``,
                  ``STREAM_END``).
                * Otherwise, the reader will emit ``STREAM_END``.
            After that ``STREAM_END``, the user may later provide ``DATA`` to
            resume reading. If this occurs, the new data will be interpreted as
            if it were at the start of the stream (i.e. it can never continue
            the previous value), except that it occurs within the same symbol
            table context. This has the following implications (where
            ``<FLUSH>`` stands for the (``INCOMPLETE``, ``NEXT``) transaction):
                * If the previously-emitted value was a numeric value (``int``,
                  ``float``, ``decimal``, ``timestamp``), the new data will
                  never extend that value, even if it would be a valid
                  continuation. For example, ``123<FLUSH>456`` will always be
                  emitted as two parse events (ints ``123`` and ``456``), even
                  though it would have been interpreted as ``123456`` without
                  the ``<FLUSH>``.
                * If the previously-emitted value was a symbol value or long
                  string, the new data will be interpreted as the start of a
                  new value. For example, ``abc<FLUSH>::123`` will be emitted
                  as the symbol value ``'abc'``, followed by an error upon
                  encountering ':' at the start of a value, even though it
                  would have been interpreted as the ``int`` ``123``
                  annotated with ``'abc'`` without the ``<FLUSH>``. The input
                  ``abc<FLUSH>abc`` will be emitted as the symbol value
                  ``'abc'`` (represented by a :class:`SymbolToken`), followed
                  by another symbol value ``'abc'`` (represented by a
                  ``SymbolToken`` with the same symbol ID), even though it
                  would have been interpreted as ``'abcabc'`` without the
                  ``<FLUSH>``.
                  Similarly, ``'''abc'''<FLUSH>'''def'''`` will the interpreted
                  as two strings (``'abc'`` and ``'def'``), even though it
                  would have been interpreted as ``'abcdef'`` without the
                  ``<FLUSH>``.
    ...
    return reader_trampoline(
        _skip_trampoline(_container_handler(None, ctx)), allow_flush=True)
