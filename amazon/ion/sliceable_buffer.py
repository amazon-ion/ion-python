...
class SliceableBuffer:
    ...
    def __init__(self, chunks=[], offset=0, size=0):
        ...
        Users should init an empty buffer and use extend to add data.
        The args exist for internal copying only.
        """
        self.chunks: List[_ChunkPair] = chunks
        # the offset is to avoid needless copying and slicing of the chunks.
        self.offset = offset
        self.size = size
    ...
    def skip(self, n):
        """
        Skip max(n, size) bytes.

        Return (skipped, new buffer).
        """
        size = self.size
        chunks = self.chunks
        offset = self.offset
