...
from amazon.ion.core import ION_STREAM_END_EVENT, IonEventType, IonEvent, \
    IonType, timestamp, OffsetTZInfo, TimestampPrecision
...
_SIMPLE_SYMBOLS_TEXT = (
    ...
    # A 'pile of poo' emoji code point.
    (u'hello\U0001f4a9world', br"'hello\U0001f4a9world'"),
)
...
SIMPLE_SCALARS_MAP_TEXT = {
    ...
    _IT.TIMESTAMP: (
        ...
        (
            _DT(2016, 1, 1, 12, 34, 12, 555000),
            b'2016-01-01T12:34:12.555000-00:00'),
        (
            _DT(2016, 1, 1, 12, 34, 12, tzinfo=OffsetTZInfo()),
            b'2016-01-01T12:34:12.000000Z'),
        (
            _DT(
                2016, 1, 1, 12, 34, 12,
                tzinfo=OffsetTZInfo(timedelta(hours=-7))),
...
SIMPLE_SCALARS_MAP_BINARY = {
    ...
    _IT.INT: (
        ...
        (
            ...
            b'\x2E\x90\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF'
            b'\xFF\xFF\xFF'),
        (
            -0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF,
            b'\x3E\x90\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF'
            b'\xFF\xFF\xFF'),
    ),
    _IT.FLOAT: (
        (None, b'\x4F'),
    ...
    _IT.TIMESTAMP: (
        (None, b'\x6F'),
        # TODO Clarify whether there's a valid zero-length Timestamp
        # representation.
        ...
        (
            timestamp(
                ...
                precision=TimestampPrecision.SECOND),
            # When fractional_precision not specified, defaults to 6 (same as
            # regular datetime).
        ...
        (
            timestamp(
                2016, 2, 2, 0, 0, 30, 100000, off_hours=-7,
    ...
    _IT.STRING: (
        ...
        (
            u'a\U0001f4a9c',
            b'\x86' + bytearray([b for b in u'a\U0001f4a9c'.encode('utf-8')])),
        (
            u'a\u0009\x0a\x0dc',
            b'\x85' + bytearray([b for b in 'a\t\n\rc'.encode('utf-8')])),
    ),
    _IT.CLOB: (
        (None, b'\x9F'),
        (b'', b'\x90'),
